...
/// with supplies for the jungle journey. Unfortunately, that Elf didn't quite follow
...
/// of each rucksack is 16 (`p`), 38 (`L`), 42 (`P`), 22 (`v`), 20 (`t`), and 19
...
/// badge is item type `B`, then all three Elves will have item type `B` somewhere
/// in their rucksack, and at most two of the Elves will be carrying any other item
/// type.
///
/// The problem is that someone forgot to put this year's updated authenticity sticker
/// on the badges. All of the badges need to be pulled out of the rucksacks so the
...
mod test {
    ...
    const PART1_EX_ANSWER: i64 = 157;
    const PART1_ANSWER: i64 = 7826;
    const PART2_EX_ANSWER: i64 = 70;
    const PART2_ANSWER: i64 = 2577;

    #[test]
    fn test_part1_ex() {
        let ex_input = std::fs::read_to_string("./inputs/day3-test.txt").unwrap();
