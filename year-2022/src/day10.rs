...
/// amount of water slowly draining from a big crack in its screen tells you it probably
/// won't be of much immediate use.
///
/// *Unless*, that is, you can design a replacement for the device's video system!
/// It seems to be some kind of [cathode-ray tube](https://en.wikipedia.org/wiki/Cathode-ray_tube)
/// screen and simple CPU that are both driven by a precise *clock circuit*. The
...
/// by the value of the `X` register) *during* the 20th cycle and every 40 cycles
...
/// position of the *middle* of that sprite. (In this system, there is no such thing
/// as "vertical position": if the sprite's horizontal position puts its pixels where
...
/// top row of pixels left-to-right, then the row below that, and so on. The left-most
/// pixel in each row is in position `0`, and the right-most pixel in each row is
/// in position `39`.
///
...
/// the CPU instructions and the CRT drawing operations, you should be able to determine
/// whether the sprite is visible the instant each pixel is drawn. If the sprite
/// is positioned such that one of its three pixels is the pixel currently being
/// drawn, the screen produces a *lit* pixel (`#`); otherwise, the screen leaves
/// the pixel *dark* (`.`).
///
...
/// ```
///
/// Allowing the program to run to completion causes the CRT to produce the following
/// image:
///
/// ```
/// ##..##..##..##..##..##..##..##..##..##..
...
pub fn part2(input: &str) -> String {
    ...
    let mut cycle = |x: i64| {
        let char = (x.dist(cycle_num % 40) < 2).choice('#', '.');
        out.push(char);
        cycle_num += 1;
        if cycle_num % 40 == 0 {
            out.push('\n');
...
const PART1_EX_ANSWER: &str = "13140";
const PART1_ANSWER: &str = "17020";
const PART2_EX_ANSWER: &str = "
##..##..##..##..##..##..##..##..##..##..
###...###...###...###...###...###...###.
####....####....####....####....####....
#####.....#####.....#####.....#####.....
