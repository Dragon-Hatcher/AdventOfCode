...
fn parse_instruction(inst: &str) -> Instruction {
    if let Some(rest) = inst.strip_prefix("cpy ") {
        let (from, to) = rest.split_once(' ').unwrap();
        Instruction::Copy(parse_val(from), parse_reg(to))
    } else if let Some(reg) = inst.strip_prefix("inc ") {
        Instruction::Inc(parse_reg(reg))
    } else if let Some(reg) = inst.strip_prefix("dec ") {
        Instruction::Dec(parse_reg(reg))
    } else {
        let rest = inst.strip_prefix("jnz ").unwrap();
...
fn main() {
    advent::new(2016, 12, default_input)
        .part1(part1)
        .part2(part2)
        .cli();
}
