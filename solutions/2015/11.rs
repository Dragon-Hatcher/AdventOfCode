...
fn is_valid_password(str: &str) -> bool {
    ...
    let Some(first_pair) = str.chars().tuple_windows().position(|(a, b)| a == b) else {
        return false;
    };
...
fn next_password(after: &str) -> String {
    let mut password = iterate_string(after);
...
fn main() {
    advent::new(2015, 11, default_input)
        .part1(part1)
        .part2(part2)
        .cli();
}
